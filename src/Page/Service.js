...
const Service = () => {
  ...
  const product = [
    { id: 1, img: "#121415", imgAlt: "Apple iPhone 16 Pro Black Titanium 128GB", discount: "Up to 15% off", rating: 5, reviews: 210, price: "€ 1.099" },
    { id: 2, img: "#333739", imgAlt: "Apple iPhone 16 Pro Natural Titanium 256GB", discount: "Up to 15% off", rating: 4.7, reviews: 210, price: "€ 1.199" },
  ...
  const [sortBy, setSortBy] = useState("default"); // Agregamos el estado para el filtro
  ...
  const filteredProducts = () => {
    if (sortBy === "price-asc") {
      return [...product].sort((a, b) => parseFloat(a.price.replace("€", "").replace(",", "")) - parseFloat(b.price.replace("€", "").replace(",", "")));
    } else if (sortBy === "price-desc") {
      return [...product].sort((a, b) => parseFloat(b.price.replace("€", "").replace(",", "")) - parseFloat(a.price.replace("€", "").replace(",", "")));
    } else if (sortBy === "rating") {
      return [...product].sort((a, b) => b.rating - a.rating);
    } else {
      return product; // Si no se selecciona filtro, mostrar todos
    }
  };
      ...
      <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
        ...
        {visibleCards < product.length && (
          <div className="w-full text-center">
            <button onClick={showMoreCards} className="rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700">
              Show More
            </button>
          </div>
        )}
